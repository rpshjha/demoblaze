...
public class DemoBlazeTest extends BaseTest {
  ...
  /** Test to validate sign up functionality. */
  @Test(priority = 2, description = "Validate Sign Up Functionality")
  public void testSignUpFunctionality() {
    TestLogger.testStart("testSignUpFunctionality");

    try {
      navigateToHomePage();
      Assert.assertTrue(homePage.isHomePageLoaded(), "Home page should load");

      String testUsername = "testuser_" + System.currentTimeMillis();
      signUpNewUser(testUsername, TestData.User.PASSWORD);

      TestLogger.assertion("Sign up completed for user: " + testUsername);
      TestLogger.info("===== TEST PASSED: Sign up functionality validated =====");

    } catch (Exception e) {
      TestLogger.error("Test failed: ", e);
      throw e;
    } finally {
      TestLogger.testEnd("testSignUpFunctionality");
    }
  }

  /** Test to validate product selection and cart functionality. */
  @Test(priority = 3, description = "Validate Product Selection and Cart")
  public void testProductSelectionAndCart() {
    TestLogger.testStart("testProductSelectionAndCart");
    ...
    try {
      ...
      TestLogger.assertion("Product selection and cart validation passed");
      TestLogger.info("Total price: $" + totalPrice);
      TestLogger.info("===== TEST PASSED: Product and cart functionality validated =====");

    } catch (Exception e) {
      TestLogger.error("Test failed: ", e);
      throw e;
    } finally {
      TestLogger.testEnd("testProductSelectionAndCart");
    }
  }
}
